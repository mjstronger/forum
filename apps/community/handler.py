    ...
    async def post(self, comment_id, *args, **kwargs):
        ...
        if form.validate():
            try:
                ...
                #写入信息
                await self.application.objects.create(Message,sender=self.current_user,receiver=replyed_user,message_type=2,
...
class ApplyHandler(RedisHandler):
    ...
    async def get(self,*args,**kwargs):
        re_data = []
        all_groups = await self.application.objects.execute(CommunityGroup.select().where(CommunityGroup.creator_id==self.current_user.id))
        all_group_ids = [group.id for group in all_groups]
        group_member_query = CommunityGroupMember.extend().where(CommunityGroupMember.community_id.in_(all_group_ids),CommunityGroupMember.status.is_null(True))
        al_members = await  self.application.objects.execute(group_member_query)
        for member in al_members:
            re_data.append({
                "user":{
                    "id":member.user.id,
                    "nick_name":member.user.nick_name,
                    "head_url":"/media/"+ str(member.user.head_url)
                },
...
class HandlerApplyHandler(RedisHandler):
    @authenticated_async
